...
import (
	...
	"github.com/Grimmer98/EcommerceCartProject/models"
...
func AddAddres() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		var size int32
		for _, value := range addressinfo {
			count, ok := value["count"].(int32) // Safely type assert
			if !ok {
				c.IndentedJSON(http.StatusInternalServerError, gin.H{"error": "Invalid data format in aggregation result"})
				return
			}
			size = count
		}
